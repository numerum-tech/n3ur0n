...
Releases : **0.1.0** (protocole initial), **0.2.0** (planner v2, BM25, GBNF, cascade), **0.3.0** (manifestes TOML, hot-reload caps, master-detail UI), **0.4.0** (open source, i18n EN/FR, RBAC phase 1, composer tous bindings, backend hot-reload), **0.4.2** (précision planner + suite d'éval, `version` dans `/health`, nommage des assets de release ; absorbe aussi ce qui avait shippé sans version en 0.4.1 : mode chat direct, couche blob, id tronqué à 20 octets, durcissement `OpenAIBackend`). **0.4.3** (adressage lisible : alias + mentions `alias#idprefix` ; lobes déclarés par instance ; sortie des capacités hors du binaire — `--manifest-dir` par défaut à `<config>/manifests`, caps utilitaires du cluster déplacées derrière le service HTTP `docker/hermes/` ; vue Activité sur `audit_log` ; tours de continuation du planner). Version workspace = `0.4.3` ; `protocol_version` fil = `n3ur0n/0.3` (inchangé). Cf. [CHANGELOG.md](CHANGELOG.md) et [ROADMAP.md](ROADMAP.md).
...
`--manifest-dir` (env `N3UR0N_MANIFEST_DIR`) **par défaut `<config-dir>/manifests`** : le mode manifeste s'engage dès que ce dossier contient un `.toml` sous `caps/` ou `backends/`, sinon le nœud garde son backend compilé (`--backend`). Le runtime manifeste est attaché dans les deux cas, donc une cap créée depuis Paramètres → Compétences est servie sans redémarrage.
...
- **`hermes`** (`docker/hermes/`, service Python stdlib, port 4250) sert les cinq fonctions utilitaires par HTTP. Ce n'est **pas** un nœud n3ur0n. node-a et node-b les atteignent par manifestes (`docker/manifests/node-{a,b}/`, montés en lecture seule sur `/etc/n3ur0n/manifests`) — les caps ne sont plus compilées dans le binaire depuis la 0.4.3, sinon tout nœud faisant tourner ce build les publierait à l'identique et le réseau n'aurait rien à router. Éditer une cap du cluster = éditer le TOML puis `docker compose restart node-a`.
...
- `N3UR0N_CAPS` (CSV) restreint ce qu'un nœud publie parmi ce que son **backend compilé** déclare — un backend compilé est tout-ou-rien. Le cluster ne s'en sert plus : node-a et node-b passent par des manifestes, où chaque cap est un fichier. Reste utile pour un nœud en mode hérité (node-c / node-d, backend `ollama`).
